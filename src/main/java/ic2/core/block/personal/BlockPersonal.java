...
public class BlockPersonal extends BlockMultiID
{
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float xOffset, float yOffset, float zOffset)
	{
		...
		else
		{
			...
			if (te == null)
				return false;
			// TODO gamerforEA add condition [6]
			else if (IC2.platform.isSimulating() && meta != 1 && meta != 2 && te instanceof IPersonalBlock && !((IPersonalBlock) te).permitsAccess(player.getGameProfile()) && !ModUtils.hasPermission(player, EventConfig.safeAccessPermission))
			{
